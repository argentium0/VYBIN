...
void main() {
  group('EncryptionService Tests', () {
    ...
    test('deriveKeyFromPassword produces consistent 32-byte key', () {
      final key1 = encryptionService.deriveKeyFromPassword(
        'myPassword123',
        'user_123',
      );
      final key2 = encryptionService.deriveKeyFromPassword(
        'myPassword123',
        'user_123',
      );
      final key3 = encryptionService.deriveKeyFromPassword(
        'differentPass',
        'user_123',
      );
    ...
    test('decryptPrivateKey with wrong key throws or fails', () async {
      final pair = await encryptionService.generateKeyPair();
      final derivedKey1 = encryptionService.deriveKeyFromPassword(
        'pass1',
        'uid_1',
      );
      final derivedKey2 = encryptionService.deriveKeyFromPassword(
        'pass2',
        'uid_1',
      );
    ...
    test(
      'encodePublicKeyToPem and decodePublicKeyFromPem restores public key',
      () async {
        final pair = await encryptionService.generateKeyPair();
